...
export function ExpensesDrawer({ from }: { from: "expenses" | "splits" }) {
  ...
  const [view, setView] = useState<ExpensesDrawerView>(EXPENSES_DRAWER_VIEW.DEFAULT);
  ...
  const content = (view: ExpensesDrawerView) => {
    switch (view) {
      case "default":
        return (
          <DrawerDefaultView members={members} onViewChange={setView} />
        );
      case "add_expense":
        return (
      ...
      case "add_member":
        return (
          <DrawerAddMemberForm
            error={addMemberMutation.error ?? null}
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content
          ...
          className={cn(
            ...
            view === "default" ? "max-w-fit" : "max-w-2xl",
          )}
        >
          <motion.div
            animate={{
